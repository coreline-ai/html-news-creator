...
> 추가 개발될 웹앱의 레이아웃 표준. 운영 화면은 워크스페이스 SaaS 표준 셸(사이드바 + 메인 + 컨텍스트 패널)을 따릅니다. 현재 정적 HTML 리포트 레이아웃은 참고 전용이며 기본 적용 대상이 아닙니다.

---

## 1. 기본 그리드

이 문서의 주 적용 대상은 **운영/관리자/대시보드 웹앱**입니다. `templates/report_newsstream.html.j2`와 `public/news/*.html`에는 사용자가 명시적으로 요청한 경우에만 별도 마이그레이션 계획을 세워 적용합니다.
...
| 일일 리포트 본문 | `max-width: 820px` | 참고 전용. 현재 HTML 템플릿의 기존 기준 |
...
현재 정적 HTML 리포트의 구조를 이해하기 위한 참고 섹션입니다. 이 디자인 시스템의 웹앱 레이아웃 규칙을 HTML 리포트에 자동 적용하지 않습니다.
