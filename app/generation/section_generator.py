...
class SectionGenerator:
    ...
    async def generate(self, cluster_id: str, items: list[dict]) -> dict:
        """
        Generate a report section for a cluster.
        items: list of {title, content_text, url, verification_status}
        Returns: {title_ko, fact_summary, social_signal_summary, inference_summary, summary_ko, importance_score, category}
        """
        if not items:
            return {
                "title_ko": "빈 클러스터",
        ...
        items_text = "\n\n".join(
            ...
            for i, item in enumerate(items[:5])  # max 5 items per cluster
        )
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": SECTION_USER.format(items=items_text)},
                ],
            ...
            return {
                "title_ko": f"클러스터 {cluster_id}",
                "fact_summary": None,
                "social_signal_summary": None,
                "inference_summary": None,
                "summary_ko": f"생성 실패: {e}",
                "importance_score": 0.0,
                "category": "other",
            }
