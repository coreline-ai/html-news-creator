...
<img width="0" height="0" alt="ChatGPT Image 2026년 4월 30일 오후 08_10_05" src="https://github.com/user-attachments/assets/9ac90ff4-3299-4ad3-b024-5df30742d3de" />
...
| 🧪 **2가지 출력 모드** | `newsstream`(기본, 기존 일간 형태) · `signal_briefing`(시그널 중심 브리핑, 메트릭·콜아웃 강조) |
| 🎨 **테마 시스템** | 색상 테마 3종(`light/dark/newsroom-white`) × `signal_briefing` 전용 6개 Refero 비주얼 프리셋 |
...
| 🛡️ **LLM 자동 복원력** | 프록시 health 사전 점검 → 다운 시 룰 기반 폴백, SDK 스트리밍 깨질 때 raw SSE 파서, 빈 응답 시 non-stream 재시도, `POST /api/system/proxy/recover`로 1-click 재기동 |
| 🚀 **배포 폴백** | Netlify 미설정 시 로컬 정적 publish로 자동 폴백 + `.logs/publish_history.jsonl`에 감사 이력 누적 |
...
│   ├── report_newsstream.html.j2        # newsstream 출력 모드 (기본)
│   ├── report_signal_briefing.html.j2   # signal_briefing 출력 모드 (6 Refero 비주얼 테마)
...
`.env` 운영 노브(선택, 기본값으로도 동작):

```env
ADMIN_RUN_DEFAULT_MAX_RUNTIME_SEC=3600         # 단일 Run hard timeout (기본 60분)
ADMIN_ALLOW_LLM_FALLBACK_ON_PROXY_DOWN=true    # 프록시 다운 시 룰 기반 폴백 허용
NEWS_STUDIO_LLM_PROXY_START_COMMAND=make proxy # /api/system/proxy/recover 가 실행할 명령
OPENAI_CHAT_STREAM=true                        # false 시 non-stream로만 호출
OPENAI_CONNECT_TIMEOUT=10                      # 초 (기본 10)
OPENAI_READ_TIMEOUT=60                         # 초 (기본 60)
OPENAI_TOTAL_TIMEOUT=120                       # 초 (기본 120)
```
...
Options:
  ...
  --output-theme TEXT  light | dark | newsroom-white  [default: dark]
  --output-style TEXT  newsstream | signal_briefing   [default: newsstream]
  --visual-theme TEXT  linear_command_center | anthropic_research_journal |
                       cursor_warm_studio | hyperstudio_terminal_ops |
                       hyperstudio_solid_dark | mercury_twilight_console
                       (signal_briefing 모드에서만 유효, 기본 hyperstudio_terminal_ops)
```
...
| 모드 | 템플릿 | 용도 |
|------|--------|------|
| `newsstream` (기본) | `templates/report_newsstream.html.j2` | 기존 일간 뉴스 형태, 섹션 카드 누적 |
| `signal_briefing` | `templates/report_signal_briefing.html.j2` | 시그널 중심 브리핑, 메트릭/콜아웃 강조 + 6가지 Refero 비주얼 프리셋 |

### 비주얼 테마 (`signal_briefing` 전용)

`hyperstudio_terminal_ops`(기본) · `hyperstudio_solid_dark` · `linear_command_center` · `anthropic_research_journal` · `cursor_warm_studio` · `mercury_twilight_console`. 화이트리스트는 [`app/rendering/visual_theme.py`](app/rendering/visual_theme.py) 단일 소스. 참고용 디자인 카탈로그 109개 마크다운은 [`demo/2026-05-01/refero-design-md/`](demo/2026-05-01/refero-design-md/)에 위치합니다.
...
- **색상 테마**: `light → dark → newsroom-white` 3단계 순환, `localStorage` 저장
...
```bash
make proxy                      # 로컬 LLM 프록시(multi-model-tui) 시작 — 별도 터미널 권장
make ui-build && make serve     # http://localhost:8000 (운영)
cd ui && npm run dev            # http://localhost:5173 (개발 HMR, 별도 uvicorn 8000 필요)
```

> 프록시가 꺼져 있어도 SPA는 정상 동작하며, `ADMIN_ALLOW_LLM_FALLBACK_ON_PROXY_DOWN=true` 기본값 덕분에 룰 기반 폴백으로 Run을 시작할 수 있습니다 (품질 저하 안내가 `SystemStatusBanner`에 노출). 프록시가 죽으면 대시보드 상단 배너의 **복구** 버튼이 `make proxy`를 자동 실행합니다.
...
| POST | `/api/reports/{date}/publish` | DB 기준 재렌더 후 Netlify 배포. **Netlify 미설정 시 로컬 정적 publish로 폴백**(200 응답, `details.target="local_static"`, `details.reason="netlify_configuration_missing"`). 모든 시도는 `.logs/publish_history.jsonl`에 JSON Line으로 누적 |
| GET | `/api/system/status` | LLM 프록시 health · `can_generate` 여부 · 복구 시나리오 노출 (대시보드 `SystemStatusBanner` 데이터 소스) |
| POST | `/api/system/proxy/recover` | `news_studio_llm_proxy_start_command`(기본 `make proxy`)를 subprocess로 실행 후 health 폴링. 결과를 `.logs/llm_proxy_recovery.log`에 기록 |
...
- **LLM 3중 폴백**: ① 사전 health check 후 다운 시 `NEWS_LLM_DISABLED=true`로 룰 기반 폴백 강제 ② SDK 스트리밍 파싱 깨질 때 raw SSE 파서로 자체 파싱 ③ 빈 스트림 응답 시 non-stream로 자동 재시도
- **LLM 호출 비용 추적**: `tiktoken`으로 input/output 토큰 카운트 → `llm_call` 이벤트로 `.logs/*.log`에 기록 (`grep llm_call .logs/*.log | jq -s 'map(.total_tokens) | add'`로 합산)
- **Run hard timeout 기본 60분**(`ADMIN_RUN_DEFAULT_MAX_RUNTIME_SEC=3600`), per-run `max_runtime_sec` 옵션으로 오버라이드 가능. SIGTERM → 5초 grace → SIGKILL 에스컬레이션
...
- **Publish는 항상 로컬 정적 결과를 먼저 확정**(`public/news/index.html` 업데이트 + DB `published_at` 스탬프)한 뒤 Netlify로 옵셔널 푸시 — Netlify 실패는 사일런트가 아니라 응답 `details.status`에 명시
- 모든 publish 시도(dry_run 포함)는 `.logs/publish_history.jsonl`에 append-only 감사 이력으로 누적. 테스트는 `NEWS_STUDIO_PUBLISH_HISTORY_PATH` 환경변수로 실 audit log 격리 가능
...
# 통합 테스트 (in-memory ASGI + SSE + render + publish smoke)
make test-integration

# Playwright E2E (News Studio golden paths)
make e2e

# 린트
make lint
make lint-design       # ui/src tailwind 토큰 일관성 검증

# 디자인 토큰 빌드 (tokens.json → tokens.css)
make build-tokens

# 로컬 LLM 프록시 (multi-model-tui) 시작 — /api/system/proxy/recover 가 호출
make proxy

# PostgreSQL 백업 (~/Backups/html-news-creator/, 30일 retention)
make backup
```
