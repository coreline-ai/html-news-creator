...
#### Tasks
- [ ] [ui/package.json](ui/package.json) 생성: `react@19`, `react-dom@19`, `react-router-dom@7`, `@tanstack/react-query@5`, `zustand@5`, `lucide-react`, `clsx`, `tailwind-merge`, `class-variance-authority`, devDeps `vite@6`, `typescript@5.7`, `@vitejs/plugin-react`, `tailwindcss@4`, `@tailwindcss/vite`, `vitest`, `@testing-library/react`
- [ ] [ui/vite.config.ts](ui/vite.config.ts): React 플러그인 + `@tailwindcss/vite` + `/api` → `http://localhost:8000` 프록시
- [ ] [ui/tsconfig.json](ui/tsconfig.json), [ui/index.html](ui/index.html), [ui/src/main.tsx](ui/src/main.tsx), [ui/src/App.tsx](ui/src/App.tsx) 골격
- [ ] [ui/src/index.css](ui/src/index.css): `@import "tailwindcss"` + `@import "../../docs/design/tokens.css"` + `<html class="dark">` 초기화 스크립트
- [ ] [ui/components.json](ui/components.json): `style: new-york`, `baseColor: neutral`, `cssVariables: true`
- [ ] shadcn 기본 12개 설치: `button card input label badge dialog dropdown-menu tabs separator scroll-area tooltip skeleton`
- [ ] [Makefile](Makefile)에 `ui-dev`, `ui-build`, `ui-test` 타겟 추가
...
#### Test Cases
- [ ] TC-0.1: dev 서버 부팅 시 콘솔 에러 0건
- [ ] TC-0.2: `<html>` 요소에 `class="dark"`가 기본 적용됨
- [ ] TC-0.3: `<Button>` 렌더 시 background에 `--primary` 토큰이 적용됨
- [ ] TC-0.E1: 잘못된 토큰 변수(`var(--unknown)`) 사용 시 fallback 색이 적용되어도 빌드는 통과
...
#### Tasks
- [ ] [app/admin/api.py](app/admin/api.py): `GET /api/reports?limit=20`, `GET /api/reports/{date_kst}` 구현 (DB의 `Report` + `ReportSection` 조회)
- [ ] [app/admin/preview.py](app/admin/preview.py): `render_preview(options: dict) -> str` — `app/editorial/policy.py`에 옵션 오버라이드 주입 후 `app/rendering/jinja_renderer.render_report()` 호출, **DB 쓰기 없이** 렌더 결과 HTML 반환
- [ ] [app/admin/api.py](app/admin/api.py): `POST /api/preview` — 위 함수 노출 (debounce는 클라이언트 책임)
- [ ] [app/admin/run_runner.py](app/admin/run_runner.py): `start_run(options) -> run_id` (asyncio.create_subprocess_exec로 `scripts/run_daily.py` 호출, stdout 라인을 큐에 push)
- [ ] [app/admin/sse.py](app/admin/sse.py): `stream_run(run_id) -> EventSourceResponse` — `sse-starlette` 사용, 단계별 progress event 전송
- [ ] [app/admin/api.py](app/admin/api.py): `GET /api/policy` (현재 yaml 반환), `PUT /api/policy` (런타임 오버라이드 저장 — 재시작 시 휘발)
- [ ] [app/admin/api.py](app/admin/api.py): `GET /api/sources`, `PUT /api/sources/{name}` (enable/disable 토글, yaml에 즉시 반영)
...
#### Test Cases
- [ ] TC-1.1: `GET /api/reports` 빈 DB → `{"reports": []}` 반환
- [ ] TC-1.2: `GET /api/reports/2026-04-30` 존재 시 14개 필드 (status, sections, generated_at 포함)
- [ ] TC-1.3: `POST /api/preview {"target_sections":5}` → HTML에 `<article class="report-section">` 5개 포함
- [ ] TC-1.4: `POST /api/runs` → 200 + `run_id` 반환, 백그라운드 프로세스 시작됨
- [ ] TC-1.5: SSE stream → 9개 step 이벤트 모두 도착 + `done` 이벤트로 종료
- [ ] TC-1.6: `PUT /api/sources/openai-blog {"enabled":false}` → `data/sources_registry.yaml` 갱신 + 200
- [ ] TC-1.E1: 존재하지 않는 날짜 `GET /api/reports/2099-01-01` → 404
- [ ] TC-1.E2: 잘못된 옵션 `POST /api/preview {"target_sections":-5}` → 400 + 메시지
...
#### Tasks
- [ ] [ui/src/lib/icons.ts](ui/src/lib/icons.ts): `SIDEBAR_NAV_ICONS = { home: Home, reports: FileText, sources: Database, policy: SlidersHorizontal, schedule: Calendar }` — **사용자가 변경할 단일 지점**
- [ ] [ui/src/components/Sidebar.tsx](ui/src/components/Sidebar.tsx): 240px 고정폭, `--sidebar-*` 토큰 사용, NavLink 5개 + 최근 실행 5건 + 하단 설정 버튼
- [ ] [ui/src/components/HeaderBar.tsx](ui/src/components/HeaderBar.tsx): 48px, 좌측 브레드크럼 / 우측 ⌘K · ThemeToggle · 계정 아이콘
- [ ] [ui/src/components/ThemeToggle.tsx](ui/src/components/ThemeToggle.tsx): `localStorage('theme')` + `documentElement.classList.toggle('dark')`, 기본 dark
- [ ] [ui/src/lib/api.ts](ui/src/lib/api.ts) + [ui/src/hooks/useReports.ts](ui/src/hooks/useReports.ts): `useReports()`, `useReport(date)` TanStack Query hooks
- [ ] [ui/src/pages/Dashboard.tsx](ui/src/pages/Dashboard.tsx): "오늘" 카드 + Quick actions 3개 카드 + 최근 실행 테이블
...
#### Test Cases
- [ ] TC-2.1: 초기 로드 시 다크 테마 활성 (background `oklch(0.145 0 0)`)
- [ ] TC-2.2: 사이드바 NavLink가 현재 경로일 때 `--sidebar-primary` 배경
- [ ] TC-2.3: 최근 실행 행 클릭 → `/reports/2026-04-30`로 이동
- [ ] TC-2.4: Theme toggle 클릭 → `dark` 클래스 제거 + localStorage `theme=light`
- [ ] TC-2.E1: API 에러 시 dashboard에 EmptyState 또는 에러 카드 표시 (백지화 금지)
...
#### Tasks
- [ ] [ui/src/lib/store.ts](ui/src/lib/store.ts): Zustand store — `runOptions`, `setOption(key, value)`, `previewMode: 'live' | 'section'`
- [ ] [ui/src/components/RunOptionsPanel.tsx](ui/src/components/RunOptionsPanel.tsx): Accordion 5그룹 (A 실행 / B 편집 / C 소스 / D 출력 / E 발행), 각 그룹에 옵션 컨트롤 + tooltip으로 `editorial_policy.yaml` 디폴트 표시
- [ ] [ui/src/hooks/usePreview.ts](ui/src/hooks/usePreview.ts): `runOptions` 변경 → 2초 디바운스 → `POST /api/preview` → HTML 반환
- [ ] [ui/src/components/LivePreview.tsx](ui/src/components/LivePreview.tsx): iframe `srcdoc=html` + 우상단 `[💻][📱][🌙]` 뷰포트/테마 토글
- [ ] [ui/src/hooks/useRunStream.ts](ui/src/hooks/useRunStream.ts): EventSource로 SSE 구독, step별 진행률 state
- [ ] [ui/src/pages/NewReport.tsx](ui/src/pages/NewReport.tsx): 좌(40%) RunOptionsPanel / 우(60%) LivePreview, 하단 sticky `[Save draft] [Run]` 액션바
- [ ] [Run] 클릭 시: 옵션을 `POST /api/runs`로 전송, 진행률 토스트, 완료 시 `ReviewReport` 페이지로 이동
...
#### Test Cases
- [ ] TC-3.1: 옵션 패널이 5개 Accordion 그룹을 렌더 (A~E)
- [ ] TC-3.2: `target_sections` 슬라이더 5→8 변경 → 2초 후 iframe 내 `<article class="report-section">` 8개
- [ ] TC-3.3: Run 클릭 → 9 progress 이벤트 수신, 마지막 `done` 이벤트 후 redirect
- [ ] TC-3.4: 미리보기 뷰포트 토글 `📱` → iframe 폭 `375px`로 축소
- [ ] TC-3.5: dry-run 토글 ON 상태에서 Run → API 응답에 `dry_run: true` 포함
- [ ] TC-3.E1: API 500 시 에러 토스트 표시 + Run 버튼 다시 활성화
- [ ] TC-3.E2: 디바운스 중 두 번째 변경 → 첫 요청 abort, 두 번째만 수행
...
#### Tasks
- [ ] [app/admin/api.py](app/admin/api.py) 추가: `PATCH /api/sections/{id}` (title/summary/implication 수정), `POST /api/sections/{id}/regenerate`, `POST /api/reports/{date}/reorder`, `POST /api/reports/{date}/publish`
- [ ] [ui/src/components/SectionList.tsx](ui/src/components/SectionList.tsx): `@dnd-kit/sortable`로 행 드래그, 행 좌측 핸들, 우측 on/off 토글, 클릭 시 SectionEditor 활성
- [ ] [ui/src/components/SectionEditor.tsx](ui/src/components/SectionEditor.tsx): 제목/이미지/요약/시사점 폼 + `[Regenerate] [Manual edit]` 버튼
- [ ] [ui/src/pages/ReviewReport.tsx](ui/src/pages/ReviewReport.tsx): 좌(40%) SectionList / 우(60%) SectionEditor or LivePreview (모드 토글)
- [ ] 미리보기 모드 토글: `live` 모드는 Phase 3와 동일 iframe / `section` 모드는 SectionEditor 폼 — store의 `previewMode`로 제어
- [ ] [Publish] 버튼: 확인 다이얼로그 → `POST /api/reports/{date}/publish` (Netlify 배포 트리거) → URL 복사
- [ ] 비활성화된 섹션(off)은 hatch 패턴으로 표시, 발행 시 자동 제외
...
#### Test Cases
- [ ] TC-4.1: SectionList 드래그 → onDragEnd 핸들러가 reorder API 호출
- [ ] TC-4.2: 섹션 off 토글 → SectionList state + 미리보기에서 해당 섹션 제거
- [ ] TC-4.3: SectionEditor에서 제목 변경 후 Save → PATCH 호출 + 미리보기 갱신
- [ ] TC-4.4: previewMode 토글 'live' ↔ 'section' → 우측 컴포넌트 전환
- [ ] TC-4.5: Publish 다이얼로그 → 확인 후 API 호출 + 토스트로 URL 표시
- [ ] TC-4.E1: 단일 섹션 regenerate 도중 에러 → 다른 섹션 영향 없음, 해당 섹션만 에러 상태
...
#### Tasks
- [ ] [ui/src/pages/Sources.tsx](ui/src/pages/Sources.tsx): 등급별 5개 그룹 헤더 + 행 (이름·tier·type·last_run·item_count·toggle·...)
- [ ] [ui/src/components/AddSourceDialog.tsx](ui/src/components/AddSourceDialog.tsx): 신규 소스 폼 (name/url/source_type/tier), 제출 시 `POST /api/sources`
- [ ] [ui/src/components/PolicyForm.tsx](ui/src/components/PolicyForm.tsx): `editorial_policy.yaml` 5섹션 (source_tiers / scoring_weights / penalties / quotas / report_selection), 슬라이더 + 숫자 입력
- [ ] 정책 저장: `PUT /api/policy` → 런타임 오버라이드 (yaml 미수정), 화면에 "변경 사항 (재시작 시 휘발)" 배너
- [ ] 소스 enable/disable 시 즉시 `data/sources_registry.yaml` 갱신, 다음 run에 반영
- [ ] [Settings] 라우트(`/settings`)는 PolicyForm을 마운트, 사이드바 하단 ⚙ 버튼에서 진입
...
#### Test Cases
- [ ] TC-5.1: Sources 페이지 첫 진입 시 37행, 5그룹 헤더
- [ ] TC-5.2: openai-blog 토글 off → yaml 파일에서 `enabled: false` 확인
- [ ] TC-5.3: PolicyForm `target_sections` 10→7 변경 + 저장 → preview API 응답이 7 섹션
- [ ] TC-5.4: AddSourceDialog 제출 → `/api/sources` 200 + 새 행 표시
- [ ] TC-5.E1: 잘못된 url(스키마 없음) → 폼 검증 에러, 제출 차단
...
#### Tasks
- [ ] `cmdk` 설치 + [ui/src/components/CommandPalette.tsx](ui/src/components/CommandPalette.tsx): ⌘K로 열고 검색·실행·이동
- [ ] [ui/src/App.tsx](ui/src/App.tsx)에 글로벌 단축키 등록: `R`(re-run last) `P`(publish current)
- [ ] 모바일 반응형: `@media (max-width: 768px)`에서 사이드바 → Sheet 변환 (`05-layout-patterns.md` 5.2)
- [ ] 모든 페이지에 EmptyState/ErrorBoundary 적용
- [ ] [ui/Makefile](Makefile) 또는 root `Makefile`: `make ui-build` → `ui/dist/` 산출, `make serve` → FastAPI가 `ui/dist`를 정적 서빙
- [ ] [app/admin/api.py](app/admin/api.py): `app.mount("/", StaticFiles(directory="ui/dist", html=True))` 단일 진입점
...
#### Test Cases
- [ ] TC-6.1: ⌘K → 검색창 표시, "오늘" 입력 → Dashboard로 이동 옵션
- [ ] TC-6.2: viewport 375px → 사이드바 hidden, ☰ 버튼 표시
- [ ] TC-6.3: API 다운(`pkill uvicorn`) → 페이지가 ErrorBoundary 메시지 + 재시도 버튼
- [ ] TC-6.4: `make ui-build` 빌드 성공 + dist 폴더 생성
- [ ] TC-6.5: `make serve` 후 `localhost:8000/sources` 새로고침 → 같은 페이지 유지 (404 아님)
...
- [ ] **시나리오 A**: Dashboard → "신규 생성" → 옵션 변경 → 라이브 미리보기 갱신 → Run → 진행률 토스트 → ReviewReport로 이동 → 섹션 1개 off → Publish → URL 복사
- [ ] **시나리오 B**: Sources에서 소스 1개 disable → NewReport 진입 → preview에 해당 소스의 섹션이 사라짐
- [ ] **시나리오 C**: Settings에서 `target_sections` 5로 저장 → NewReport preview에 5 섹션
- [ ] **시나리오 D**: 다크 → 라이트 토글 → 모든 페이지에서 토큰이 라이트 값으로 전환
