...
매일 37개 활성 AI 뉴스 소스를 수집·분류·클러스터링해 **개발자 중심 AI 트렌드 HTML 리포트**를 자동 생성하는 파이프라인.
...
| 파일 | 역할 |
|------|------|
| `scripts/run_daily.py` | 파이프라인 전체 오케스트레이터 (1,500줄+) |
| `data/sources_registry.yaml` | 37개 활성 소스 설정 (RSS / GitHub / arXiv / HN / Reddit / YouTube) |
...
| `app/models/db_models.py` | SQLAlchemy ORM 14개 테이블 |
