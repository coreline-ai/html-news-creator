...
> **프로젝트 적용 주의**
> 이 문서는 디자인 에이전트 운영 표준이다. `docs/design/**`에 정의된 현재 프로젝트 디자인 시스템은 **추가 개발될 웹앱 UI**를 위한 것이며, 현재 생성되는 정적 HTML 리포트(`templates/report_newsstream.html.j2`, `public/news/*.html`)의 스타일을 대체하지 않는다.
> 루트에 위치하지만 HTML 뉴스 리포트 템플릿의 토큰/스타일 가이드가 아니므로, 명시적 마이그레이션 요청 없이 리포트 CSS에 자동 적용하지 않는다.
